  ...
  return (
    <div className="fixed inset-0 z-[999] bg-gradient-to-br from-slate-900 via-indigo-950 to-black flex flex-col items-center justify-center overflow-hidden">
      <div
        ...
        style={{ backgroundImage: `url(${remoteUser?.profile_picture})` }}
      />
      ...
      <p className="z-10 text-slate-300 mt-2">@{remoteUser?.username}</p>
      <div className="z-10 mt-6 flex items-center gap-2">
        <div className="w-3 h-3 rounded-full bg-green-400 animate-pulse"></div>
        <span className="text-green-300 font-medium">Connected • {formatTime()}</span>
      </div>
      <div className="absolute bottom-16 flex gap-10 z-10">
        <button
          ...
          className={`w-16 h-16 rounded-full flex items-center justify-center transition duration-300 shadow-xl
            ${isMuted ? "bg-red-500 hover:bg-red-600" : "bg-white/15 backdrop-blur-lg hover:bg-white/25"}`}
        >
          {isMuted ? <MicOff className="text-white" size={28} /> : <Mic className="text-white" size={28} />}
        </button>
        <button
