      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        rounded-md p-1 bg-white shadow-sm'>
          {
            ...
            (<button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm
              rounded-md cursor-pointer transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black'
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item) => item.label === currentTab).value.map((user) => (
            <div className='w-full max-w-88 flex gap-5 p-6 bg-white
            shadow rounded-md' key={user._id}>
              ...
              <div className='flex-1'>
                ...
                <div className='flex mx-sm:flex-col gap-2 mt-4'>
                  {
                    <button onClick={() => navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600
